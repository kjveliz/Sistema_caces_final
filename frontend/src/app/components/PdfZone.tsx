...
interface PdfZoneProps {
  ...
  onFileSelected?: (slot: EvidenceSlot, file: File) => void;
  disabled?: boolean;
  loading?: boolean;
}
...
export default function PdfZone({
  ...
  return (
    <div
      ...
      style={{
        borderColor: hasFile ? '#16A34A' : hasError ? '#DC2626' : 'rgba(27,58,107,0.15)',
        background: hasFile ? '#F0FDF4' : hasError ? '#FEF2F2' : '#F8FAFD',
    ...
    >
      <div className="flex items-center gap-2.5">
        <div className="flex-shrink-0">
          {hasFile ? (
            ...
            <FileText size={15} style={{ color: '#5A7295' }} />
          )}
        </div>

        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold leading-snug" style={{ color: '#0F1E3C' }}>
            {slot.label}
          </p>
        ...
        {!disabled && (
          <button
            ...
            style={{
              background: hasFile ? '#16A34A' : '#1B3A6B',
              color: '#fff',
            }}
          >
            {loading ? <Loader2 size={10} className="animate-spin" /> : <Upload size={10} />}

            {loading ? 'Cargando...' : hasFile ? 'Cambiar' : 'Subir'}
          </button>
        )}
      </div>
      ...
      <input
        ref={inputRef}
        type="file"
        accept={esCsv ? '.csv,text/csv' : '.pdf,application/pdf'}
        className="hidden"
        onChange={pick}
