...
export default function LoginView({ onLogin }: LoginViewProps) {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const respuesta = await iniciarSesion(email.trim(), pass);

      if (!respuesta.ok) {
        setErr(respuesta.mensaje || 'No se pudo iniciar sesión.');
        return;
      }

      onLogin(respuesta.usuario);
    } catch (error) {
      setErr(error instanceof Error ? error.message : 'No se pudo conectar con el servidor.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: 'linear-gradient(150deg,#0F2556 0%,#1B3A6B 55%,#1E4A8A 100%)',
        }}
      >
        <div>
          ...
          <p className="text-blue-200 text-sm leading-relaxed max-w-xs">
            Modelo de evaluación para Unidades Académicas de Formación Técnica y Tecnológica - CACES
          </p>
        </div>
          ...
          {[
            ...
            >
              <div
                ...
                style={{
                  background: index === 0 ? '#60A5FA' : 'rgba(255,255,255,0.35)',
                }}
              />
      ...
      >
        <div className="w-full max-w-md">
          ...
          >
            ...
            >
              Acceso exclusivo para personal autorizado de la Universidad Católica de Santiago de
              Guayaquil.
            </p>
          </div>
        </div>
