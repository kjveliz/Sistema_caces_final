...
export default function CareersView({
  ...
  const [newCareerModalidad, setNewCareerModalidad] =
  useState("");
  ...
  const [editCareerModalidad, setEditCareerModalidad] =
    useState("");
  ...
  const deletableCareers = deleteArea
    ...
    : [];
  function organizarCarrerasPorArea(
  ...
  return AREAS.map((area) => {
    const carrerasDelArea = carrerasBD
      .filter(
        (carrera) =>
          carrera.area_conocimiento.trim().toLowerCase() ===
      ...
      .map((carrera) => {
         ...
         */
        const carreraAnterior = AREAS.flatMap(
          (areaAnterior) => areaAnterior.careers,
        ).find(
        ...
        const career: Career = {
          ...
          code: carrera.codigo,
          criterionNum:
            carreraAnterior?.criterionNum ?? 4,
          /*
  ...
  async function cargarCarreras() {
    try {
      ...
      const areasActualizadas =
        organizarCarrerasPorArea(carrerasConsultadas);
  ...
  function seleccionarCarreraParaEditar(id: string) {
    ...
    const carrera = carrerasBD.find(
      (item) => Number(item.id_carrera) === Number(id),
    );
  ...
  async function handleDeleteCareer(
    ...
    const carreraSeleccionada = carrerasBD.find(
      (carrera) =>
        carrera.codigo.toUpperCase() ===
        deleteCareer.toUpperCase(),
    );

    if (!carreraSeleccionada) {
      toast.error(
        "No se encontró la carrera seleccionada en la base de datos.",
      );
      return;
    }
    ...
    try {
      ...
      await eliminarCarrera(
        (carreraSeleccionada.id_carrera),
      );
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    value={editCareerId}
                    onChange={(event) =>
                      seleccionarCarreraParaEditar(event.target.value)
                    }
                    required
                    disabled={updatingCareer}
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setEditCareerName(event.target.value)
                    }
                    required
                    disabled={updatingCareer}
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(event) =>
                        setEditCareerArea(event.target.value)
                      }
                      required
                      disabled={updatingCareer}
                      className="w-full px-3 py-2 rounded-xl text-sm border outline-none appearance-none cursor-pointer"
                ...
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={(event) =>
                        setEditCareerModalidad(event.target.value)
                      }
                      required
                      disabled={updatingCareer}
                      className="w-full px-3 py-2 rounded-xl text-sm border outline-none appearance-none cursor-pointer"
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={(event) =>
                      setNewCareerModalidad(event.target.value)
                    }
                    required
                    className="w-full px-3 py-2 rounded-xl text-sm border outline-none appearance-none cursor-pointer"
                    style={{
