...
import { ArrowLeft, CheckCircle2, ChevronDown, FileText, Loader2 } from 'lucide-react';
...
export default function EvidenceUploadView({
  ...
  async function procesarPdf(slot: EvidenceSlot, archivo: File) {
    ...
    if (indicator.id === 'I2' && I2_SLOT_TIPO[slot.sourceNum] !== undefined) {
      if (asignaturaId === null) {
        toast.error(
    ...
    if (indicator.id === 'I3' && I3_SLOT_TIPO[slot.sourceNum] !== undefined) {
      if (asignaturaId === null) {
        toast.error(
          'No se pudo determinar la asignatura. Verifique la configuración de período y materia.',
        );
    ...
    return (
      ...
      >
        <EvidenceHeader
          ...
          onBackClick={() => (preselectedIndicatorId ? onBack() : setStep('selectIndicator'))}
        />
        <div className="flex-1 flex items-center justify-center px-6">
          <div className="w-full max-w-lg">
            ...
            <button
              onClick={() => canContinue && setStep('upload')}
        ...
        >
          <div className="max-w-3xl mx-auto px-6 h-14 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button
                onClick={() => setStep(isSyllabus ? 'configSyllabus' : 'configTitDes')}
            ...
            <span
              ...
              style={{ color: done === total ? '#16A34A' : '#5A7295' }}
            >
              {done}/{total}
            </span>
          </div>
