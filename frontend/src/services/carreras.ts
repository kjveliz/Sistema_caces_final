...
const API_BASE =
  "http://localhost/sistemacaces/api/carreras";
...
export async function crearCarrera(
  ...
  const resultado =
    await leerRespuestaJson<RespuestaCrearCarrera>(
      respuesta,
...
export async function eliminarCarrera(
  ...
  if (!Number.isInteger(idCarrera) || idCarrera <= 0) {
    throw new Error(
      "El identificador de la carrera no es válido.",
    );
  }
  ...
  const resultado =
    await leerRespuestaJson<RespuestaEliminarCarrera>(
      respuesta,
...
export async function actualizarCarrera(
  ...
  const resultado =
    await leerRespuestaJson<RespuestaActualizarCarrera>(
      respuesta,
...
export async function subirMallaCurricular({
  ...
  const nombreArchivo =
    `Malla_Curricular_${carrera.codigo}.pdf`;
  ...
  const drive =
    await leerRespuestaJson<SubirMallaDriveResponse>(
      respuestaDrive,
  ...
  if (
    !respuestaDrive.ok ||
    !drive.ok ||
    !drive.datos?.id_archivo ||
    !drive.datos.url_archivo
  ) {
    throw new Error(
      drive.mensaje ??
        drive.detalle ??
        "No se pudo subir la malla a Google Drive.",
    );
  }

  const respuestaGuardar = await fetch(
    `${API_BASE}/guardar_malla.php`,
    {
      method: "POST",
      credentials: "include",
      headers: {
      ...
      body: JSON.stringify({
        ...
        nombre_archivo:
          drive.datos.nombre_archivo || nombreArchivo,
  ...
  const guardado =
    await leerRespuestaJson<GuardarMallaResponse>(
      respuestaGuardar,
