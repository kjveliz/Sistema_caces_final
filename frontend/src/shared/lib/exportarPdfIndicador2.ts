...
const EF_INFO: Record<
  'ef1' | 'ef2' | 'ef3' | 'ef4' | 'ef5',
  { titulo: string; descripcion: string; peso: number; fuente: string }
> = {
  ef1: {
    titulo: 'EF1',
    descripcion: 'Seguimiento de contenidos del syllabus',
    peso: 33,
    fuente: 'Encuesta de heteroevaluación, syllabus y malla curricular',
  },
  ef2: {
    titulo: 'EF2',
    descripcion: 'Mejora al micro currículo',
    peso: 27,
    fuente: 'Documentos de planificación y actas de resolución',
  },
  ef3: {
    titulo: 'EF3',
    descripcion: 'Proceso de seguimiento difundido',
    peso: 20,
    fuente: 'EVA, informes y registros de difusión',
  },
  ef4: {
    titulo: 'EF4',
    descripcion: 'Difusión del syllabus en el EVA',
    peso: 13,
    fuente: 'EVA — carga y difusión del syllabus',
  },
  ef5: {
    titulo: 'EF5',
    descripcion: 'Normativa institucional',
    peso: 7,
    fuente: 'Reglamento interno de seguimiento',
  },
};
...
function fuente(doc: PdfDoc, rol: FontRole, sizePt: number, color: RGB) {
  switch (rol) {
    case 'serif':
      ...
      break;
    case 'serifItalic':
      doc.setFont('SourceSerif4', 'italic');
      break;
    case 'serifBold':
      doc.setFont('SourceSerif4SemiBold', 'normal');
      break;
    case 'sans':
      doc.setFont('PlexSans', 'normal');
      break;
    case 'sansItalic':
      doc.setFont('PlexSans', 'italic');
      break;
    case 'sansMedium':
      doc.setFont('PlexSansMedium', 'normal');
      break;
    case 'sansBold':
      doc.setFont('PlexSansSemiBold', 'normal');
      break;
    case 'mono':
      doc.setFont('PlexMono', 'normal');
      break;
    case 'monoMedium':
      doc.setFont('PlexMonoMedium', 'normal');
      break;
  }
  doc.setFontSize(sizePt);
  doc.setTextColor(color[0], color[1], color[2]);
}
function fill(doc: PdfDoc, c: RGB) {
  doc.setFillColor(c[0], c[1], c[2]);
}
function stroke(doc: PdfDoc, c: RGB) {
...
function drawRunHeader(doc: PdfDoc, paginaNum: number) {
  ...
  doc.text(String(paginaNum).padStart(2, '0'), MARGIN_X + CONTENT_W, MARGIN_TOP + logoH - 1.5, {
    align: 'right',
  });
...
function drawTablaEFs(
  ...
  efKeys.forEach((key, i) => {
    ...
    const numerosPregunta =
      key === 'ef1' || key === 'ef4'
        ? encuestaDetalle.preguntas
            .filter((p) => (key === 'ef1' ? p.es_ef1 : p.es_ef4))
            .map((p) => `P${p.numero}`)
        : [];
...
function drawValidityNote(doc: PdfDoc, y: number): number {
  ...
  doc.text(
    'Documento generado automáticamente por el sistema de seguimiento académico UCSG - No requiere firma.',
    MARGIN_X + 2.6,
    y + 1.6,
  );
  return y + 6;
...
function drawAnexoPreguntas(
  ...
  encuestaDetalle.preguntas.forEach((p, i) => {
    ...
    if (dom) {
      ...
      doc.text(`${dom.pct}% - n=${total}`, centroResultado, y + rowH / 2 + 2.8, {
        align: 'center',
      });
    } else {
...
export async function construirDocumentoIndicador2(datos: DatosPdfIndicador2): Promise<PdfDoc> {
  ...
  y = dividerConTitulo(
    ...
    y,
    CONTENT_W,
    'Elementos fundamentales y fuentes de evidencia',
  );
  ...
  y = dividerConTitulo(
    ...
    y,
    CONTENT_W,
    'Anexo — Las 23 preguntas de la encuesta de heteroevaluación',
  );
  y = drawSectionSub(
    doc,
    MARGIN_X,
    y,
    CONTENT_W,
    'Distribución de respuestas por pregunta. Las filas marcadas con EF alimentan directamente el cálculo del indicador.',
  );
